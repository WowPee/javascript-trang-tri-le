...
function posLeft() {
    return "undefined" != typeof window.pageXOffset ? window.pageXOffset : document.documentElement.scrollLeft ? document.documentElement.scrollLeft : document.body.scrollLeft ? document.body.scrollLeft : 0
}
...
function stopfire() {
    for (var t = firestop.length - 1; t >= 0; t--) {
        clearInterval(firestop[t]);
        for (var e = fire[t].sp.length - 1; e >= 0; e--) fire[t].sp[e].style.visibility = "hidden"
    }
}

function fireworks() {
    new fo(50), setTimeout("new fo(50)", 750), stopafter > 0 && setTimeout("stopfire()", 6e4 * stopafter)
}
...
layernum = 0, piece = function(t) {
    if (this.elem = null, nsDOM) null == t ? this.elem = new Layer(1) : (this.elem = new Layer(1, t.elem), this.style.visibility = "inherit"), this.parent = t, this.style = this.elem;
        ...
        null == t ? this.parent = document.body : this.parent = t.elem, this.elem = document.createElement("div");
        var e = "xLayer" + layernum++;
        this.elem.setAttribute("id", e), elemc = document.createTextNode("."), this.elem.appendChild(elemc), this.parent.appendChild(this.elem), this.style = this.elem.style, document.getElementById(e).style.lineHeight = "3px", document.getElementById(e).style.zIndex = "999999999999999999", document.getElementById(e).style.color = "#fff", document.getElementById(e).style.position = "absolute"
    }
    window[this.elem.id] = this, this.ay = .1, this.type = 0
    ...
    var i = t > 25 ? Math.random() * (380 - 5 * t) : 255 - 4 * t,
        s = d2h(i - 112);
    112 > i && (s = d2h(i)), this.sCol(d2h(i), s, e), this.dy += this.ay, this.x += this.dx, this.y += this.dy, this.moveTo(this.x, this.y)
}, fo = function(t) {
    ...
    if (this.step > 55 && (this.step = 0), 0 == this.step) {
